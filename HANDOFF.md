...
## 2026-08-16／claude（擬真品質測試——簡繁轉換/標點修正/Playwright 全流程/三尺寸版面）

本輪不查文件、直接餵真實文稿驗品質。四項逐一記錄：

**A. 簡繁轉換品質**：用 30 個常見簡繁詞彙（软件/信息/视频/网络/数据库/服务器/鼠标/U盘/硬盘/内存/
打印机/文件夹/用户/菜单/默认/云计算/人工智能/算法/编程 等）＋一段長句實測 opencc(cn→twp)+覆蓋層
管線，**全部正確**（软件→軟體、信息→資訊、U盘→隨身碟、云计算→雲端計算、算法→演算法…）。
人名（习近平→習近平、马云→馬雲）也正確逐字轉換（無 NER，符合預期，非缺陷）。
**觀察**（非 bug）：opencc 的 `twp` 對「台」一律標準化為「臺」（台灣→臺灣），
與專案自己 165 條覆蓋層／文件中慣用的「台」字風格不一致；這是 opencc 內建行為，
不在本專案可控範圍，供之後若要做覆蓋層補丁參考。
**繁→簡反向轉換：不支援**——`app/page.tsx`／`lib/textProcessor.ts` 只有 `simplified-to-traditional`
一種 FeatureType，全專案 grep 無任何 T2S 實作，非本輪缺陷，是既有功能範圍。

**B. 標點修正品質**：抓到一個**真 bug**並已修復——見下方「真問題」。其餘情況全部正確：
半形逗號句號→全形、問號驚嘆號dedupe（？？？→？）、直角引號→彎引號（"..."→「...」、'...'→『...』）、
全形括號轉換、標點前後多餘空白清除、段落結尾自動補句號、中文標點+英文自動補空格。

**C. 操作流程（Playwright，臨時裝在 /tmp、未動 package.json）**：起 dev server 走完整流程——
貼入 550 字含簡繁詞彙的文稿 → 勾選簡體轉繁體+修正標點 → 處理 → 右側 diff 正確顯示轉換結果
（軟體/資訊/資料庫等）→ 清除 → 切到 AI 輔助編輯分頁 → 輸入假 API Key → `page.route` 攔截
`https://api.openai.com/v1/chat/completions` 回傳 mock 內容（未打真 API）→ AI 處理結果正確顯示 →
複製按鈕可用。全程 **0 個 JS console error**。

**D. 版面可用性**：375px／768px／1440px 三尺寸皆**無橫向捲軸**（scrollWidth==clientWidth，
含長文稿+diff 顯示中也一樣）；2200 字長文稿處理耗時僅 **65ms**（opencc 因 `preloadDictionaries()`
已在頁面載入時背景預載，不會卡頓）；三尺寸截圖目視確認排版無重疊/裁切，手機版 checkbox 直列、
按鈕自動換行，皆正常。

**真問題與修復（真 bug，已修＋補迴歸測試）**：
`lib/processors/fixPunctuation.ts` 的「省略號→⋯⋯」規則（原第 58-59 行）是**死碼**且會製造亂碼。
根因是規則順序：第 2 步的「重複標點符號去重」（`。{2,}`→單一`。`、`\.\.{1,}`→`。`）比省略號規則
先跑，導致連續 3 個以上的點永遠等不到被省略號規則看見——因為它們早被前面的規則拆散、
各自轉換掉了。實測：`"等等..."`（使用者最常見的打法）修正後變成 **`"等等。。"`**（兩個全形句點的
亂碼），`"省略號...继续说下去...."` 變成 **`"省略號。.。继续说下去。。"`**（殘留半形句點混雜全形句點，
更明顯的亂碼）。**修法**：把省略號偵測搬到函式最開頭（新第 0 步），在任何逐點轉換/去重規則
跑之前，先把連續 3 個以上的半形或全形句點整組收斂成 `⋯⋯`，原本死碼的第 8 步隨之刪除。
`lib/processors/fixPunctuation.test.ts` 新增 3 條迴歸測試（半形三/四點、全形三點、混合雙省略號
不留殘餘半形點）。

**測試**：`npm test` 12 檔 **95 → 98 全綠**；`npm run lint` 乾淨；`npm run build` 成功；
`node test-functions.mjs` 37/37；`node test-dictionaries-simple.mjs` 4/4（此腳本的 Test 4
是自己重寫的簡化標點邏輯、不是真的呼叫 `fixPunctuation.ts`，本輪未動它，僅供資料完整性煙霧測試，
非本輪缺陷）。
