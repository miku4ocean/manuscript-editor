...
- 驗收現況：`npm test` 84/84 通過；`npm run build` 成功且 Route 摘要只剩靜態頁（無 `ƒ` Dynamic）；`npm run lint` 0 errors、4 warnings（均為舊有未用變數，與改動邏輯無關）
- **已 commit**（2026-07-27）：12 個 lint 錯誤已修復（fixTypos const、scripts ES imports、types unknown）
